...
def signal_handler(signal, frame):
    ...
    save_dataframe(df, './Data/Human_generated_data.xlsx')
    exit(0)
...
def process_prompt(cell_content, index):
    instruction = "You are a virtual grading assistant. Directly provide a numeric score explicitly formatted as 'Score:[number]'. Do not include your thinking process in the output."
    ...
    try:
        ...
        if score is not None:
            df.at[index, 'Deepseek-R1_0-Shot'] = float(score)
            df.at[index, 'Deepseek-R1_0-Shot Output'] = response
...
with ThreadPoolExecutor(max_workers=10) as executor:
    ...
    for future in as_completed(futures):
        ...
        df.at[index, 'Deepseek-R1_0-Shot'] = score
        df.at[index, 'Deepseek-R1_0-Shot Output'] = full_output
        count += 1
        if count % 500 == 0:
            temp_output_path = './Data/HumanTEMP_generated_data.xlsx'
...
final_output_path = './Data/DeepseekHuman_generated_data.xlsx'
